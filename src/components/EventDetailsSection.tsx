...
import { getEvents, getZones, type EventItem } from "@/lib/domainApi";

const EventDetailsSection = () => {
  const [events, setEvents] = useState<EventItem[]>([]);
  const [zoneMasters, setZoneMasters] = useState<any[]>([]);
                ...
                <div className="rounded-2xl overflow-hidden shadow-card bg-muted min-h-[300px] p-6">
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {event?.categoryZoneMappings?.length && zoneMasters?.length ? (
                        event.categoryZoneMappings
                          .map((mapping) => {
                            const zone = zoneMasters.find(
                              (z) =>
                                z._id?.toString() === mapping.zoneId?.toString()
                            );
